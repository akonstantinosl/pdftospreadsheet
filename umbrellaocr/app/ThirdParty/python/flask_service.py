...
def process_pdf(file_path):
    ...
    try:
        ...
        app.logger.error(f"Could not get PDF info for {file_path}: {e}")
        raise ValueError(f"Failed to read PDF file info. It might be corrupted or password-protected.")
    ...
    for page_num in range(1, page_count + 1):
        try:
            ...
            img_array = np.array(pdf_image[0].convert('RGB'))

            page_tables = process_image_for_tables(img_array)
            if page_tables:
                all_tables.extend(page_tables)

            # Pembersihan memori eksplisit di dalam loop
            del pdf_image
            del img_array
            if 'page_tables' in locals():
                del page_tables
            gc.collect()

        except Exception as e:
            app.logger.error(f"Error processing page {page_num} of {file_path}: {e}")
            # Pastikan untuk membersihkan memori bahkan jika terjadi error pada satu halaman
            if 'pdf_image' in locals(): del pdf_image
...
def process_file():
    ...
    try:
        ...
        if file_extension == 'pdf':
            ...
            tables = process_image(input_filepath)
        else:
        ...
        if not tables:
            if os.path.exists(input_filepath):
                os.remove(input_filepath)
            return jsonify({'success': False, 'message': 'No tables could be extracted from the document.'}), 404

        export_data = create_tables_export(tables, output_format)

        base_name_from_upload = os.path.splitext(filename)[0]
        output_extension = output_format
        if output_format == 'csv' and len(tables) > 1:
            output_extension = 'zip'
        
        output_filename = f"processed_{base_name_from_upload}.{output_extension}"
        output_filepath = os.path.join(SHARED_FOLDER, output_filename)
        
        with open(output_filepath, 'wb') as f:
            f.write(export_data)

        # Hapus file input asli setelah berhasil diproses
        if os.path.exists(input_filepath):
            os.remove(input_filepath)
        ...
        # Membersihkan variabel yang dialokasikan selama permintaan ini.
        del tables
        del export_data
        gc.collect()
        ...
        app.logger.error(f"Processing error for {data.get('filename', 'unknown file') if data else 'unknown file'}: {e}", exc_info=True)
        # Pastikan file input dihapus jika terjadi error
        if input_filepath and os.path.exists(input_filepath):
            os.remove(input_filepath)
        # Lakukan garbage collection bahkan saat terjadi error
        gc.collect()
